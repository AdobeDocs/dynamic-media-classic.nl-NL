...
>[!NOTE]
>
>Als u een sjabloon bewerkt die in een eerdere versie van Adobe Dynamic Media Classic is gemaakt, wordt u gevraagd of u de sjabloon wilt opslaan. Wilt u een canvaslaag toevoegen? Selecteer **[!UICONTROL No]** als u geen basislaag wilt toevoegen. Als u per ongeluk **[!UICONTROL Yes]** selecteert, verwijdert u de opties `&allowCanvasPrompt` en `&layer=0` in de URL en drukt u op **[!UICONTROL Enter]** of **[!UICONTROL Return]** .

## De eerste sjabloon maken {#creating-the-initial-template}

Wanneer u een sjabloonset maakt, heeft de optie **[!UICONTROL Publish after a save]** op de volgende manieren invloed op de set en stelt u leden in:
...
**om het aanvankelijke malplaatje tot stand te brengen:**

1. Gebruik een van de volgende methoden om uw oorspronkelijke sjabloon te maken:

   * **selecteer eerst de PSD of de beelden**: In het Browse paneel, selecteer het dossier of de beelden van PSD u voor uw malplaatje wilt, ga **[!UICONTROL Build]** > **[!UICONTROL Template Basics]**.

   * **Begin van het scherm van het Malplaatje**: Ga naar **[!UICONTROL Build]** > **[!UICONTROL Template Basics]**. Selecteer Designer of Developer.

1. Voer in het dialoogvenster Canvasgrootte invoeren de afmetingen voor breedte en hoogte van de sjabloon in.
1. Selecteer een map in de Asset Library en sleep het PSD-bestand of de afbeeldingen die u voor de sjabloon wilt gebruiken naar het scherm Sjabloon.
...
Of u nu een gepubliceerde set of een niet-gepubliceerde sjabloonset bewerkt, de optie **[!UICONTROL Publish after a save]** heeft op de volgende manieren invloed op de set en stelt leden in:
...
1. Blader in de rasterweergave naar een Sjabloonset en selecteer vervolgens **[!UICONTROL Edit]** onder de afbeelding.
...
1. Selecteer **[!UICONTROL Apply]** en sluit het tekstscherm.
...
1. Geef een of meer van de volgende opmaakopties op en selecteer vervolgens **[!UICONTROL Apply]** .
   ...
   * **[!UICONTROL Font Size]**: Kies een tekengrootte in het menu, typ een specifieke grootte in het vak of selecteer de **[!UICONTROL Up]** - of **[!UICONTROL Down]** -pijlen om de grootte met twee punten te vergroten of te verkleinen.
...
>[!NOTE]
>
>Selecteer **[!UICONTROL Undo]** als u de laatste handeling wilt omkeren. Selecteer **[!UICONTROL Redo]** als u van gedachten verandert over het omkeren van een handeling nadat u **[!UICONTROL Undo]** hebt geselecteerd.
...
1. Geef een of meer van de volgende opmaakopties op en selecteer vervolgens **[!UICONTROL Apply]** .
   ...
   * **[!UICONTROL Alignment]** - Geef het type uitlijning op door op **[!UICONTROL Align left]** , **[!UICONTROL Align center]** , **[!UICONTROL Align right]** of **[!UICONTROL Justify]** te klikken.

   * **[!UICONTROL End of Paragraph Justification]**: selecteer deze optie om het type uitvulling op te geven voor de laatste regel in de alinea: de laatste regel wordt links uitgelijnd, de laatste regel wordt gecentreerd en de laatste regel wordt rechts uitgelijnd.
   ...
   * **[!UICONTROL Indent All]**: selecteer deze optie om de mate waarin de tekst inspringt, te verhogen.

   * **[!UICONTROL Remove Indent]**: selecteer deze optie om de mate waarin de tekst inspringt, te verlagen.
...
1. Selecteer in het deelvenster Laageigenschappen een van de volgende opties:
   ...
   * **[!UICONTROL Wrap]**: hiermee loopt de tekst om in een te klein horizontaal tekstvak te passen.

   * **[!UICONTROL No Wrap]**: laat de tekst niet omlopen wanneer het tekstvak te klein horizontaal is, en knipt in plaats daarvan een gedeelte van de tekst weg.
...
De informatie op het tabblad Source van de Teksteditor is ter referentie. Bewerk de tekst alleen als u bekend bent met het bewerken van broncode.
...
1. Bekijk of bewerk de tekst naar wens.
   ...
   Wijzigingen blijven intact als u heen en weer schakelt tussen de voorvertoning en de Source-weergave.
...
* **manueel veranderende grootte**: Selecteer en sleep een hoek van de laag of het canvas. Met tekstlagen kunt u ook een zijde van de laag slepen. Houd Shift ingedrukt terwijl u sleept om de grootte te wijzigen, maar de hoogte-breedteverhouding (de vorm) te behouden.

* **het ingaan van metingen van de laaggrootte**: Ga pixelmetingen in de de tekstvakjes van B (Breedte) en van H (Hoogte) op het gebied van de Eigenschappen van de Laag in.
...
* **Veranderend de positie van een laag**: Om de positie van een laag op het canvas te veranderen, selecteer zijn naam in de lijst van Lagen en gebruik één van deze technieken:

* **manueel veranderende positie**: Beweeg de wijzer dichtbij maar niet over een laaggrens, en wanneer u de vier-geleide pijlcurseur ziet, en begint te slepen.

* **het ingaan van de metingen van de plaatscompensatie**: Ga X en Y verschuivingsmetingen in de de tekstvakjes van X en van Y in. Deze metingen vertegenwoordigen de x- en y-verschuiving van het ankerpunt in pixels.

* **het Roteren van een laag**: De Rotate doos maakt een lijst van de hoek waaraan de laag werd geroteerd. Als u een laag wilt roteren, selecteert u de naam van de laag in de lijst Lagen en gebruikt u een van de volgende technieken:

* **manueel het roteren**: Beweeg de curseur dichtbij maar niet over een hoek van de laag. Wanneer u de rotatiecursor ziet, sleept u de hoek van de laag. Houd Shift tijdens het slepen ingedrukt om in stappen van 15 graden te roteren.

* **die een graadmeting** ingaat: Ga het aantal graden in om de laag te roteren. De rotatie is rechtsom. Als u de selectie linksom wilt roteren, voert u een negatief getal in.

**verberg een laag of een laageffect:**

U kunt een laag- of laageffect verbergen door het oogpictogram naast de naam van een laag of effect te selecteren. Verborgen lagen worden niet weergegeven in voorvertoningen of uitvoer. De laaggegevens worden niet verwijderd uit de URL. In plaats daarvan wordt `hide=1` aan de URL toegevoegd om te zien dat de laag verborgen is. Bijvoorbeeld:
...
* **Voorgrondkleur**: Selecteer **[!UICONTROL Foreground Color]** en kies een kleurenmonster om de kleur van de schaduw of gloed te veranderen. U kunt ook een parameter color-value in het vak invoeren. De achtergrondkleur is alleen van toepassing op lagen die transparantie gebruiken. Het is bijvoorbeeld van toepassing op een gedeeltelijk transparante laag in een prijstag of de achtergrond van een tekstveld. Lagen die bestaan uit een PSD-, TIFF- of PNG-afbeelding waarop transparantie is ingeschakeld, kunnen een transparante achtergrond hebben.

* **Achtergrondkleur**: Selecteer **[!UICONTROL Background Color]** en kies een kleurenmonster om de kleur van de opgevulde gebieden te veranderen.

* **Opaciteit**: Sleep de schuif van de Opaciteit om het even welke laag doorzichtig te maken zodat een deel van het onderliggende beeld door toont. De instelling van 100 procent is dekkend en 0 is transparant.

* **overvloeiwijze**: Om één van de mengselwijzen te simuleren beschikbaar in Photoshop, kies een optie. De opties zijn Normaal, Verspreiden, Lichter, Donkerder, Vermenigvuldigen en Scherm. Deze opties zijn beschikbaar voor lagen, niet voor het canvas.

## Schaduw- en gloedeffecten gebruiken op lagen {#using-shadow-and-glow-effects-on-layers}

U kunt een schaduw of gloed op een laag toepassen. De schaduw of gloed wordt toegepast op de omtrek van de laag en wordt naar binnen of naar buiten uitgebreid, afhankelijk van de optie voor schaduw of gloed die u kiest. Als uw sjabloon afkomstig is uit een PSD-bestand met schaduw- en gloedeffecten, kunt u deze effecten aanpassen in Adobe Dynamic Media Classic.
...
1. Selecteer het menu **[!UICONTROL `Add Effect`]** en kies een optie:
...
* **[!UICONTROL Opacity]**: Sleep de schuifregelaar om te bepalen hoe intens het effect is. Minder dekkende effecten zijn transparanter.
...
* **[!UICONTROL Size]**: voer metingen in in het vak X en Y om het schaduweffect te vergroten of te verkleinen. Grootteopties zijn alleen beschikbaar voor binnenschaduwen en slagschaduwen.
...
De lijst Lagen bevat een knop Masker die aangeeft hoe het masker of het alfakanaal van een laag wordt gebruikt. Met de knop Masker kunt u het effect van een achtergrondlaag toepassen op een bepaalde laag of op de gehele bovenliggende laag in de sjabloon. Selecteer een laag in de lijst Lagen en selecteer **[!UICONTROL Mask]** om deze frames te doorlopen:
