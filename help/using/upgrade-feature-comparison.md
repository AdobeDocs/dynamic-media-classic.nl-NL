---
title: Functievergelijking
description: De vergelijkingslijst van de eigenschap tussen  [!DNL Dynamic Media]  in  [!DNL Adobe Experience Manager]  Assets en  [!DNL Adobe Dynamic Media Classic]  Desktoptoepassing.
feature: Dynamic Media Classic
role: Admin,User
exl-id: eb56533a-4e31-4367-8b68-879cd70c761e
topic: Content Management
level: Intermediate
source-git-commit: 8dc990a1fb1355b00fa4839e14b92bb6562d40b4
workflow-type: tm+mt
source-wordcount: '160'
ht-degree: 0%

---

# Functievergelijking

Een tabel waarin de functies [!DNL Dynamic Media] worden vergeleken in [!DNL Adobe Experience Manager] Assets en [!DNL Adobe Dynamic Media Classic] bureaubladtoepassing. Een &quot;✓&quot;betekent de eigenschap in of met de software beschikbaar is; a &quot;-&quot;betekent het niet.

| Functie | [!DNL Dynamic Media] | [!DNL Dynamic Media<br> Klassiek &#x200B;] |
| :--- | :---: | :---: |
| 360-video | ✓ | ✓ |
| Geavanceerde verwerking van afbeeldingen | ✓ | ✓ |
| Geavanceerde video (bijschriften, SEO) | ✓ | ✓ |
| API&#39;s | ✓ | ✓ |
| Automatisch instellen genereren | ✓ | ✓ |
| CDN-integratie | ✓ | ✓ |
| Dynamic Imaging | ✓ | ✓ |
| eCatalogs | ✓<br>*Assets + InDesign Server* | ✓ |
| Meteen publiceren | ✓ | ✓ |
| Interactieve viewers | ✓ | ✓ |
| Bestanden manipuleren | ✓ | ✓ |
| Media Portal | ✓<br>*gekend als &quot;Brand Portal&quot;* | ✓ |
| Regelsets | ✓ | ✓ |
| Beveiligde voorvertoning | ✓ | ✓ |
| Sets (Afbeelding, Draaien, Gemengde media) | ✓<br>*Geen multiaxis spins* | ✓ |
| Slimme afbeeldingen | ✓ | ✓ |
| Statische levering van inhoud <br> (PDF, GIT, enzovoort) | ✓ | ✓ |
| Videolevering (adaptief) | ✓ | ✓ |
| Videocodering | ✓ | ✓ |
| Toegankelijkheid van viewer | ✓ | ✓ |
| Viewer Integration met Analytics | ✓ | ✓ |
| 1 :1 Sjablonen | ✓<br>*Gepersonaliseerde media bundel* | ✓ |
| Visual Configurator <br> (productverpersoonlijking) | ✓<br>*Gepersonaliseerde media bundel* | ✓ |
| 3D: Experience Manager Sites <br> component WCM | ✓ | : |
| 3D: Interactive Experience Viewer | ✓ | : |
| 3D: AIR/VR en belangrijkste 3D-bestanden publiceren | ✓ | : |
| Carrousels | ✓ | : |
| Verwisselbare/interactieve media: afbeeldingen | ✓ | : |
| Schoppbare/interactieve media: video | ✓ | : |
| Slim uitsnijden: afbeeldingen | ✓ | : |
| Slim uitsnijden: Video | ✓ | : |
| Selectie videominiatuur | ✓ | : |
| Viewer Designer | ✓ | : |
| YouTube Publiceren | ✓ | : |
