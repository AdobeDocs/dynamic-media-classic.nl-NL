...
U kunt informatie opslaan specifiek voor de dossiers u met in Adobe Dynamic Media Classic werkt; deze informatie wordt genoemd *meta-gegevens*. U kunt metagegevens in Adobe Dynamic Media Classic gebruiken voor het ordenen, zoeken, filteren en sorteren van uw elementen.
...
* **Compacte Mening**: Een basislijst van waarden.

* **IPTC**: Waarden zoals die door de Internationale Raad van de Telecommunicatie van de Pers worden bepaald.

* **XMP**: Waarden zoals die door het verlengbare meta-gegevensprogramma worden bepaald.
...
1. Open het deelvenster Metagegevens en voer een of beide van de volgende handelingen uit:
   ...
   * Kies een vooraf ingestelde waarde en selecteer vervolgens **[!UICONTROL Apply]** om metagegevensvelden te vullen met vooraf ingestelde waarden. Bedrijfsbeheerders maken deze vooraf ingestelde waarden.

1. Voer de waarden in het deelvenster Metagegevens in.

>[!NOTE]
>
>Als u de metagegevens van meerdere elementen tegelijk wilt bewerken, selecteert u de elementen en gaat u naar **[!UICONTROL File]** > **[!UICONTROL Edit Info]** . De bewerkingen die u hebt aangebracht in metagegevens in het venster Info bewerken, worden toegepast op alle elementen die u hebt geselecteerd.
...
1. Selecteer **[!UICONTROL Keywords]** .
1. Voer een van de volgende handelingen uit om trefwoorden toe te voegen:
   ...
   * Selecteer een trefwoord in de tabel **[!UICONTROL Keyword Suggestions]** .

1. Als u een trefwoord wilt verwijderen, selecteert u het en selecteert u **[!UICONTROL Remove]** . Het beweegt zich aan de lijst van Suggesties van het Sleutelwoord.
...
1. Selecteer **[!UICONTROL Upload Metadata]** in het dialoogvenster **[!UICONTROL Browse]** .
1. Selecteer in het dialoogvenster **[!UICONTROL Select files to upload]** het tabgescheiden of XML-bestand met de metagegevens.
...
* Namen van door de gebruiker gedefinieerde velden worden geïdentificeerd als gemaakt in **[!UICONTROL Setup]** > **[!UICONTROL Application Setup]** > **[!UICONTROL Metadata]** > **[!UICONTROL User-Defined fields]** . Gebruik de functie `Generate file` om een lijst met alle gedefinieerde PDF&#39;s in de juiste importindeling op te halen.
* Eigenschappen van XMP-metagegevens moeten het gerelateerde XMP-voorvoegsel hebben vóór de naam (eigenschap-). Een dubbele punt scheidt het voorvoegsel en de naam. U vindt het voorvoegsel voor XMP in **[!UICONTROL Setup]** > **[!UICONTROL Application Setup]** > **[!UICONTROL Metadata]** > **[!UICONTROL Metadata Schema]** Editor. De technische namen vindt u in de documentatie van het gerelateerde XMP-schema. Namen van XMP-eigenschappen worden niet weergegeven in de functie `Generate file` .
* Eigenschappen van Metagegevensschema moeten het verwante voorvoegsel vóór de naam (eigenschap-) hebben. Een dubbele punt scheidt het voorvoegsel en de naam. Het voorvoegsel en de eigenschapnamen worden gedefinieerd in de Metadata Schema Editor. Namen van eigenschappen van het metagegevensschema worden niet weergegeven in de functie `Generate file` .

Bijvoorbeeld: de XMP-eigenschap voor trefwoorden is het XMP-schema &quot;Dublin Core&quot; met het voorvoegsel `dc` en `subject` is de technische XMP-naam. Het voorvoegsel en de technische XMP-naam worden gecombineerd in de volledige eigenschapnaam `dc:subject` . In de indeling voor het importeren van XML-metagegevens moet `dc.subject` de naam van de eigenschap zijn. In de door tabs gescheiden importindeling moet dit de kolomkop zijn.
...
Een bestand voor het importeren van metagegevens met de waarde `Hello\, World!,back\\slash,foo` for `dc:subject` stelt bijvoorbeeld drie XMP-trefwoorden in op het element: `Hello, World!,` `back\slash,` en `foo` .

### XMP- en Metagegevensschema-XMP-bestanden importeren

De XML-import accepteert alleen geldige XML. Bij het importeren van XMP- of metagegevensschemavelden wordt het naamruimtevoorvoegsel toegevoegd en gedraagt dit zich als een XMP-naamruimte. Deze naamruimte moet worden gedeclareerd. Bijvoorbeeld in de tag op hoofdniveau.
...
U kunt metagegevens voor meerdere bestanden importeren. U voert de metagegevens in een door tabs gescheiden of XML-bestand in. Selecteer vervolgens **[!UICONTROL Process metadata files]** op de pagina Taakopties uploaden (via het tabblad FTP).
...
Selecteer **[!UICONTROL Upload]** op de algemene navigatiebalk. Als u de metagegevens wilt importeren, selecteert u op de pagina Uploaden de tab **[!UICONTROL Via FTP]** en vervolgens **[!UICONTROL Job Options]** . Selecteer **[!UICONTROL Job]** in het dialoogvenster Taakopties uploaden en schakel vervolgens het selectievakje **[!UICONTROL Process metadata files]** in.
...
Om identiteitskaart van Adobe Dynamic Media Classic van een beeld anders te noemen, voeg een kolom geëtiketteerd *genummerd* aan het lusje-afgebakende dossier toe, of voeg een geroepen gebied `new_vc_objectname` aan de gegevens van XML toe.
...
1. Ga naar **[!UICONTROL File]** > **[!UICONTROL Import Metadata]** .
1. Selecteer **[!UICONTROL Asset Properties Type]** voor **[!UICONTROL Image]** .
1. Kies **[!UICONTROL `Generate File`]** , **[!UICONTROL Tab-delimited Template]** of **[!UICONTROL Asset's XML Metadata]** in de vervolgkeuzelijst **[!UICONTROL XML DTD]** .
1. Selecteer **[!UICONTROL Generate]** .
...
| Verwijderen | Hiermee verwijdert u een eigenschap of keuzevrijheid uit het schema. Waarden worden niet verwijderd uit het XMP-blok of de database. De eigenschap is niet meer beschikbaar voor weergaven van metagegevens en wordt verwijderd uit de weergave Details van element. Als het bezit aan de Server van Meta-gegevens werd gepubliceerd, voer een kracht uit publiceert om de gegevens uit de openbaar-onder ogen ziet Server van Meta-gegevens te verwijderen. |
...
* **[!UICONTROL Name]**: UI-naam voor het schema. Hiermee identificeert u de eigenschappen in Metagegevensweergaven en Geavanceerd zoeken. Vergelijkbaar met XMP-secties zoals Basic, IPTC en PDF.

* **[!UICONTROL Prefix]**: Technische unieke id voor het schema. Beperkt tot de letters a-z en A-Z. Het voorvoegsel is niet zichtbaar in de gebruikersinterface van Adobe Dynamic Media Classic, maar wordt gebruikt wanneer metagegevens voor een element zijn opgeslagen in het XMP-blok en de database. Het voorvoegsel identificeert uniek meta-gegevensgebieden in meta-gegevensonderzoek vragen op de Server of de invoer van Meta-gegevens.

* **[!UICONTROL Namespace]**: Technische unieke id voor het schema, doorgaans een URL in de vorm `https://your.company.com/name/version/` . Zie de lijst met standaardschema&#39;s voor voorbeelden. De naamruimte is niet zichtbaar in de gebruikersinterface van Adobe Dynamic Media Classic, maar wordt gebruikt om metagegevens op te slaan in het XMP-blok.
...
De eigenschappen beschrijven de meta-gegevens die met dit schema in het blok van XMP kunnen worden opgeslagen. Een eigenschap bestaat uit:

| Eigenschap | Beschrijving |
| --- | --- |
| ID | Technische id voor deze eigenschap. De id is niet zichtbaar in de gebruikersinterface van Adobe Dynamic Media Classic, maar wordt gebruikt wanneer metagegevens voor een element zijn opgeslagen in het XMP-blok en de database. De id wordt gebruikt om zoekopdrachten te maken op de Metagegevensserver. ID heeft sommige beperkingen zoals: `<ul><li>No spaces</li><li>No ".", ":", "$"</li><li>No number as first character</li><li>Best practice is to use a-z or A-Z as first character</li></ul>` <br> Zodra gecreeerd, identiteitskaart kan niet worden veranderd. |
...
| Eigenschap | Beschrijving |
| --- | --- |
| ID | Technische identificatie voor deze waarde. De id is niet zichtbaar in de gebruikersinterface van Adobe Dynamic Media Classic, maar wordt gebruikt wanneer metagegevens voor een element zijn opgeslagen in het XMP-blok en de database. De id wordt gebruikt in zoekopdrachten op de Metagegevensserver. De id mag geen spaties bevatten. Nadat de id is gemaakt, kan deze niet worden gewijzigd. |
